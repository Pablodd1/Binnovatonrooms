    ...
    try {
      ...
      if (!raw) {
        ...
        if (attempt < GEMINI_RETRY_COUNT) {
          await new Promise((r) =>
            setTimeout(r, GEMINI_RETRY_DELAY_MS * attempt),
          );
          continue;
        }
      ...
      log.info(
        { attempt, durationMs, rawLength: raw.length },
      ...
      log.error(
        { attempt, durationMs, error: message },
...
function detectDetailLevel(
  ...
  const hasHighRes =
    qualityNotes.includes("P") ||
    qualityNotes.includes("2MP") ||
    qualityNotes.includes("4MP");
...
function mergeDiagnoses(
  ...
  if (
    second.severidad === "critica" ||
    (first.severidad !== "critica" && second.severidad === "alta")
  ) {
  ...
  const evidenceSet = new Set([
    ...first.evidencia_visual,
    ...second.evidencia_visual,
  ]);
  merged.evidencia_visual = [...evidenceSet].slice(0, 12);

  const indicatorSet = new Map(
    first.visual_indicators.map((ind) => [
      `${ind.label}-${Math.round(ind.x)}-${Math.round(ind.y)}`,
      ind,
    ]),
  );
  for (const ind of second.visual_indicators) {
    ...
    if (
      !indicatorSet.has(key) ||
      ind.confidence > (indicatorSet.get(key)?.confidence || 0)
    ) {
  ...
  if (
    second.causa_probable &&
    second.causa_probable.length > first.causa_probable.length
  ) {
  ...
  const solutionSet = new Set([
    ...first.solucion_paso_a_paso,
    ...second.solucion_paso_a_paso,
  ]);
  merged.solucion_paso_a_paso = [...solutionSet].slice(0, 8);

  const measurementSet = new Set([
    ...first.mediciones_recomendadas,
    ...second.mediciones_recomendadas,
  ]);
...
function mergeYoloIntoDiagnosis(
  ...
  const existingKeys = new Set(
    merged.visual_indicators.map(
      (ind) => `${ind.label}-${Math.round(ind.x)}-${Math.round(ind.y)}`,
    ),
  );
  ...
  const yoloEvidences = detections.map(
    (d) =>
      `YOLO detection: ${d.defect_type} (${Math.round(d.confidence * 100)}% confidence)`,
  );
...
export async function POST(request: Request) {
  ...
  if (imageEntries.length === 0) {
    return NextResponse.json(
      { error: "Upload at least one image field named image or images." },
      { status: 400 },
    );
  }
  ...
  if (imageEntries.length > MAX_ANALYSIS_IMAGES) {
    return NextResponse.json(
      { error: `Use ${MAX_ANALYSIS_IMAGES} images or fewer per inspection.` },
      { status: 400 },
    );
  }

  const totalImageBytes = imageEntries.reduce(
  ...
  if (totalImageBytes > MAX_TOTAL_IMAGE_BYTES) {
    return NextResponse.json(
      {
        error:
          "Inspection image set is too large. Keep total uploads under 30MB.",
      },
      { status: 413 },
    );
  }
  ...
  for (const image of imageEntries) {
    ...
    if (imageError) {
      return NextResponse.json(
        { error: imageError },
        { status: imageError.includes("large") ? 413 : 400 },
      );
    }
  }

  const cameraLabel = sanitizeText(
    formData.get("cameraLabel"),
    "unknown camera",
    160,
  );
  ...
  const requestedDetailLevel = formData.get(
    "detailLevel",
  ) as DetailLevel | null;
  const detailLevel =
    requestedDetailLevel && DETAIL_LEVELS.includes(requestedDetailLevel)
  ...
  const imagePayloads = await Promise.all(
    imageEntries.map((image) => fileToGeminiPart(image)),
  );
  ...
  const [yoloResult, geminiResult] = await Promise.all([
    detectDefectsBatch(imageBuffers, {
      ...
      log.warn(
        ...
        "YOLO detection failed, continuing with Gemini only",
      );
      return null;
    }),
    (async () => {
      ...
      if (!firstPassRaw) {
        ...
        return {
          error: "AI model returned no diagnosis.",
          status: 502 as const,
        };
      }
      ...
      try {
        ...
        log.error(
          { raw: firstPassRaw.slice(0, 200) },
          "Failed to parse first pass response",
        );
        return {
      ...
      if (
        ...
        (detailLevel === "detailed" || detailLevel === "forensic")
      ) {
        log.info(
          "Starting second pass analysis for multi-image forensic review",
        );
        try {
          ...
          if (result.raw) {
            const secondPassDiagnosis = JSON.parse(
              result.raw,
            ) as InspectionDiagnosis;
            finalDiagnosis = mergeDiagnoses(
              firstPassDiagnosis,
              secondPassDiagnosis,
            );
            log.info(
              {
                firstPassConfidence: firstPassDiagnosis.confianza,
                secondPassConfidence: secondPassDiagnosis.confianza,
              },
          ...
          log.warn(
            { error: error instanceof Error ? error.message : "Unknown" },
            "Second pass failed, using first pass results",
          );
        }
      }
  ...
  if (yoloResult && yoloResult.detections.length > 0) {
    ...
    log.info(
      {
        yoloDetections: yoloResult.detections.length,
        depthAvailable: !!firstDepth,
      },
      "Merging YOLO detections into diagnosis",
    );
    finalDiagnosis = mergeYoloIntoDiagnosis(
      finalDiagnosis,
  ...
  // Optimize array filtering: single loop for multiple categorizations using regex
  const micro_defects_detected: string[] = [];
  const environmental_factors: string[] = [];
  const microRegex = /micro|fina|incipiente|incip/;
  const envRegex = /humedad|temperatura|condensacion/;

  for (let i = 0; i < finalDiagnosis.riesgos.length; i++) {
    const r = finalDiagnosis.riesgos[i];
    if (microRegex.test(r)) micro_defects_detected.push(r);
    if (envRegex.test(r)) environmental_factors.push(r);
  }

  const surface_conditions: string[] = [];
  const surfaceRegex = /superficie|textura|acabado/;
  for (let i = 0; i < finalDiagnosis.mediciones_recomendadas.length; i++) {
    const m = finalDiagnosis.mediciones_recomendadas[i];
    if (surfaceRegex.test(m)) surface_conditions.push(m);
  }
  ...
  const [imageUrls, installers] = await Promise.all([
    Promise.all(
      imagePayloads.map((payload) =>
        storeImage(payload.bytes, payload.mimeType),
      ),
    ),
