...
function parseAnalyzeRequest(formData: FormData) {
  const imageEntries = [
    formData.get("image"),
...
async function runGeminiAnalysis(
  client: GoogleGenAI,
  model: string,
  imagePayloads: { bytes: Buffer; mimeType: string; part: any }[],
  cameraLabel: string,
  locationLabel: string,
  lidarNotes: string,
  qualityNotes: string,
...
export async function POST(request: Request) {
  ...
  const parsedRequest = parseAnalyzeRequest(formData);
  ...
  finalDiagnosis.analysis_metadata = {
    ...
    environmental_factors: finalDiagnosis.riesgos.filter(
      (r) =>
        r.includes("humedad") ||
        r.includes("temperatura") ||
        r.includes("condensacion"),
    ),
  };
  ...
  const qualityPayload = {
    ...
    imageCount: imageEntries.length,
    totalImageBytes,
    brightness: qualityPayloadInput.brightness,
    sharpness: qualityPayloadInput.sharpness,
    glarePercent: qualityPayloadInput.glarePercent,
    contrast: qualityPayloadInput.contrast,
    grade: qualityPayloadInput.grade,
    detailLevel,
