...
function extractColorAndLighting(data: Uint8ClampedArray) {
  let brightness = 0;
  let brightPixels = 0;
  let darkPixels = 0;
...
function extractTexture(gradientMagnitudes: number[]) {
  ...
  return { hasFineTexture, hasSmoothSurface, smoothPercent };
}
...
function extractHistogramSpread(grayscale: number[]) {
...
function evaluateFrameQuality(
  ...
  return { checks, grade, status, guidance, notes };
}
...
function scoreFrame(canvas: HTMLCanvasElement): QualityScore {
  ...
  const {
    ...
    hasColorCast,
    grayscale,
  } = extractColorAndLighting(data);
  const { gradientMagnitudes, sharpness, centerDetailRatio } = extractEdges(
    grayscale,
    sampleWidth,
    sampleHeight,
  );
  const { hasFineTexture, hasSmoothSurface, smoothPercent } =
    extractTexture(gradientMagnitudes);
  const noiseEstimate = estimateNoise(grayscale, sampleWidth, sampleHeight);
  const histogramSpread = extractHistogramSpread(grayscale);
  const megapixels = (canvas.width * canvas.height) / 1_000_000;

  const { checks, grade, status, guidance, notes } = evaluateFrameQuality(
    brightness,
    sharpness,
    contrast,
    glarePercent,
